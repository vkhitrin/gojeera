...
class DebugInfoScreen(ExtendedModalScreen[None]):
    ...
    def _get_main_screen(self) -> JiraApp | None:
        return cast('JiraApp', self.app)

    def _build_executed_query(self, main_screen: JiraApp, search_data: dict | None) -> str | None:
        if not search_data:
            return None
        ...
        if mode in ('text', 'jql') and not main_screen._freeform_search_query(search_data):
            return None
        ...
        if mode == 'basic' and not executed_query:
            ...
            if project_key == Select.NULL:
                project_key = None
            if assignee == Select.NULL:
                assignee = None
            if work_item_type == Select.NULL:
                work_item_type = None
            if status == Select.NULL:
                status = None
