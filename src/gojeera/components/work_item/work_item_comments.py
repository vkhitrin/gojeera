...
def _handle_comment_screen_result(
    ...
    *,
    ...
    resolved = _resolve_comment_screen_result(result)
    if resolved is None:
        return None
...
class CommentContainer(Vertical, can_focus=False):
    ...
    def _can_modify_comment(self, action: str) -> bool:
        ...
        if not (self._comment_author_id and current_user_id == self._comment_author_id):
            self.notify(
                f'You can only {action} your own comments',
                severity='warning',
                title=self._work_item_key,
            )
            return False
    ...
    def handle_edit_result(self, result: dict[str, object] | str | None) -> None:
        comment = _handle_comment_screen_result(
            self,
            result,
            work_item_key=self._work_item_key,
...
class CommentsScrollView(VerticalScroll):
    ...
    def select_comment_at(self, index: int, *, scroll_into_view: bool = True) -> bool:
        ...
        self._selected_comment_id = containers[index]._comment_id

        if previous_index != index:
            if 0 <= previous_index < len(containers):
                containers[previous_index].remove_class('-selected')
            containers[index].add_class('-selected')
        else:
            containers[index].add_class('-selected')
    ...
    def action_new_comment(self) -> None:
        widget = self._get_comments_widget()
        if widget is not None and widget.can_add_comment:
    ...
    def check_action(self, action: str, parameters: tuple[object, ...]) -> bool | None:
        ...
        if action == 'new_comment':
            widget = self._get_comments_widget()
            return widget.can_add_comment if widget is not None else False
        return True
...
class WorkItemCommentsWidget(Vertical, can_focus=False):
    ...
    comments: Reactive[list[WorkItemComment] | None] = reactive(None, always_update=True)
    ...
    def work_item_is_service_desk(self) -> bool:
        return self._work_item_is_service_desk

    @work_item_is_service_desk.setter
    def work_item_is_service_desk(self, value: bool):
        self._work_item_is_service_desk = value

    @property
    def content_container(self) -> Container:
        return self.query_one(
            '.tab-content-container',
            expect_type=Container,
        )
    ...
    def can_add_comment(self) -> bool:
        ...
        return (
            ...
            or not (permission_response.error or '').startswith(
                MISSING_ADD_COMMENT_PERMISSION_ERROR_PREFIX
            )
        )

    def compose(self) -> ComposeResult:
        with Container(classes='tab-content-container') as content:
            content.display = True

            yield CommentsScrollView(
                id='comments-scroll-view',
    ...
    def save_comment(self, result: dict[str, object] | str | None) -> None:
        comment = _handle_comment_screen_result(
            ...
            success_message='Comment added successfully' if self.work_item_key else None,
        )
        if comment is None:
            return
    ...
    def action_add_comment(self) -> None:
        if self.work_item_key:
            self.run_worker(
                self._open_add_comment_screen_if_permitted(self.work_item_key),
                exclusive=True,
    ...
    async def _open_add_comment_screen_if_permitted(self, work_item_key: str) -> None:
        ...
        await self.app.push_screen(
            CommentScreen(
                ...
                work_item_is_service_desk=self.work_item_is_service_desk,
            ),
            self.save_comment,
        )
    ...
    def _restore_comment_selection(self, scroll_view: CommentsScrollView) -> None:
        selected_comment_id = scroll_view._selected_comment_id
        if selected_comment_id:
            index = self._comment_indices.get(selected_comment_id)
            if index is not None:
                scroll_view.select_comment_at(index, scroll_into_view=False)
                return
        if self._comment_containers:
            scroll_view.select_comment_at(0, scroll_into_view=False)
    ...
    def _apply_pending_comment_focus(self) -> None:
        ...
        if not self._comment_indices and self.comments:
            return
