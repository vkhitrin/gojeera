...
class WorkItemInfoContainer(Container, can_focus=False):
    ...
    def __init__(self):
        ...
        self._description_loading_worker = None
    ...
    def content_container(self) -> Container:
        return self.query_one('.tab-content-container', expect_type=Container)

    def compose(self) -> ComposeResult:
        with Container(classes='tab-content-container'):
            with VerticalScroll(can_focus=True, classes='tab-scroll-surface'):
                yield WorkItemDescription()

    def _finalize_description_render(self, work_item_key: str) -> None:
        current_work_item = self.work_item
        if current_work_item is None or current_work_item.key != work_item_key:
            return
        self.description_container.styles.opacity = 1
    ...
    async def _setup_work_item_description(self, work_item: JiraWorkItem) -> None:
        ...
        if work_item.description:
            ...
            if self.work_item is None or self.work_item.key != work_item.key:
                return
            ...
            self.call_after_refresh(self._finalize_description_render, work_item.key)
        else:
            self.description_container.remove_class('required')
    ...
    def watch_work_item(self, work_item: JiraWorkItem | None) -> None:
        ...
        self.description_container.visible = True
        self.work_item_description_widget.visible = True
        self.description_container.styles.opacity = 0
    ...
    def _start_setup_work_item_description_worker(self, work_item_key: str) -> None:
        ...
        existing_worker = self._description_loading_worker
        if existing_worker is not None and not existing_worker.is_finished:
            existing_worker.cancel()

        self._description_loading_worker = self.run_worker(
            self._setup_work_item_description(current_work_item)
        )
    ...
    def _call_coordinated_loading(self) -> None:
        try:
            hide_loading = getattr(self.app, '_try_hide_loading_coordinated', None)
