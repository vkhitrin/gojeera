...
class JiraConfig(BaseSettings):
    ...
    active_profile_name: str | None = Field(
        default=None,
    ...
    def api_email(self) -> str | None:
        ...
        return self.active_profile.basic_email() if self.active_profile is not None else None
    ...
    def cloud_id(self) -> str | None:
        ...
        return self.active_profile.oauth_cloud_id() if self.active_profile is not None else None

    @property
    def oauth2_client_id(self) -> str | None:
        if self.oauth2_client_id_override is not None:
            return self.oauth2_client_id_override
        return self.active_profile.oauth_client_id() if self.active_profile is not None else None
    ...
    def has_auth_configuration(self) -> bool:
        ...
        return any(
            value is not None
            for value in (
                self.auth_type_override,
                self.api_email_override,
                self.api_token,
                self.oauth2_access_token,
                self.oauth2_refresh_token,
                self.api_base_url_override,
                self.cloud_id_override,
                self.oauth2_client_id_override,
                self.oauth2_client_secret,
                self.oauth2_scopes_override,
            )
        )
...
class ApplicationConfiguration(BaseSettings):
    ...
    def validate_jira_config(cls, v):
        ...
        if isinstance(v, dict):
            if 'profiles' in v:
                v = v.copy()
                v['profiles'] = normalize_profiles(v['profiles'])
...
class KeyringSettingsSource(InitSettingsSource):
    ...
    def __call__(self) -> dict[str, object]:
        ...
        if isinstance(jira_config, dict):
            active_profile = resolve_current_profile(jira_config)
