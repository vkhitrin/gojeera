...
[https://gojeera.readthedocs.io/en/latest/index.html](https://gojeera.readthedocs.io/en/latest/index.html)
...
gojeera supports a few ways to search work items.
...
In addition to searching using the filters above, gojeera allows you to search items using full-text
...
length of the search term to activate the search. Independently of the value you enter for this variable gojeera imposes
...
Another way to search work items in gojeera is by crafting your own [JQL query](). You can do so using the JQL Query
...
By default gojeera will retrieve all available projects. However, if you set the config variable
...
If you select a project then gojeera will retrieve all the applicable issue types for the selected project. If no
...
If you select a project then gojeera will retrieve all the applicable statis codes applicable to the issue types of the
...
If you select a project then gojeera will retrieve all the users that can be assigned issues in the given
...
- `textfield`: these fields allow the user to provide a simple string as value. No Markdown or ADF is supported by
  these fields. **Important**: this is a restriction of the type as defined by Jira and not a restriction of gojeera.
...
By default, gojeera does not allow users to view and update the fields in the above. To enable this feature you can set
the variable `enable_updating_additional_fields: True` in the config file. For more details refer to
[Enable Updating Additional Fields](https://gojeera.readthedocs.io/en/latest/users/configuration/configuration.html#enable-updating-additional-fields)
...
gojeera disables the parent field of an issue when its type does not allow parents to be set; e.g. for Epics.
