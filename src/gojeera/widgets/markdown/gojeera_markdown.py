...
from gojeera.utils.jira.urls import WORK_ITEM_BROWSE_TOOLTIP, extract_work_item_key
from gojeera.utils.markdown.mdit_adf_decision import decision_plugin
from gojeera.utils.markdown.mdit_adf_panels import panels_plugin
...
def _map_strip_styles(
    strip: Strip,
    style_mapper: Callable[[RichStyle | None], RichStyle | None],
) -> Strip:
    segments = [
        Segment(
            text,
...
def _style_meta_value(style: RichStyle | None, key: str) -> object | None:
    if style is None or style.meta is None:
        return None
    return style.meta.get(key)
...
def apply_focused_link_style_to_strip(
    ...
    def map_style(style: RichStyle | None) -> RichStyle | None:
        if _style_meta_value(style, ATTACHMENT_REFERENCE_FILENAME_META_KEY) is not None:
            return cast(RichStyle, style) + RichStyle(underline=False)
        if _style_meta_value(style, MARKDOWN_LINK_HREF_META_KEY) == focused_link_href:
            return cast(RichStyle, style) + link_hover_style
        return style

    return _map_strip_styles(strip, map_style)
...
def apply_attachment_hover_style_to_strip(
    ...
    def map_style(style: RichStyle | None) -> RichStyle | None:
        if (
            _style_meta_value(style, ATTACHMENT_REFERENCE_FILENAME_META_KEY)
            == focused_attachment_filename
        ):
            return cast(RichStyle, style) + attachment_hover_style
        return style
...
def apply_attachment_chip_style_to_strip(
    ...
    def map_style(style: RichStyle | None) -> RichStyle | None:
        if _style_meta_value(style, ATTACHMENT_REFERENCE_FILENAME_META_KEY) is not None:
            return cast(RichStyle, style) + attachment_chip_style
        return style
...
def apply_inline_code_style_to_strip(
    ...
    def map_style(style: RichStyle | None) -> RichStyle | None:
        inline_code_meta = _style_meta_value(style, INLINE_CODE_META_KEY)
        if isinstance(inline_code_meta, bool) and inline_code_meta:
            return cast(RichStyle, style) + inline_code_style
        return style
...
class AttachmentTooltipProvider:
    ...
    def _find_attachment(self, filename: str | None) -> 'Attachment | None':
        ...
        attachments_widget = getattr(self.markdown.app, 'work_item_attachments_widget', None)
    ...
    def get(self, filename: str | None) -> Text:
        ...
        size_text = f'{attachment.get_size()} KB'
...
class ExtendedMarkdownParagraph(MarkdownParagraph):
    ...
    def __init__(
        ...
        super().__init__(markdown, token, source_range=source_range)
    ...
    def watch_hover_style(self, previous_hover_style: RichStyle, hover_style: RichStyle) -> None:
        ...
        if attachment_filename is not None:
            ...
            cast('JiraApp', self.app).focused_work_item_link_key = None
            return

        work_item_key = get_markdown_link_work_item_key(hover_style)
        cast('JiraApp', self.app).focused_work_item_link_key = work_item_key
    ...
    async def _on_click(self, event: events.Click) -> None:
        ...
        if event.ctrl:
            ...
            if work_item_key is not None:
                load_work_item = getattr(self.app, 'load_work_item', None)
                if callable(load_work_item):
                    result = load_work_item(work_item_key)
                    if isawaitable(result):
                        event.prevent_default()
                        event.stop()
                        self.app.run_worker(result, exclusive=True)
                        return

        await super()._on_click(event)

    def action_attachment(self, filename: str) -> None:
        navigate_to_attachment = getattr(self.app, 'navigate_to_attachment', None)
...
class GojeeraMarkdownFence(MarkdownFence):
    ...
    def highlight(cls, code: str, language: str, ansi: bool = False, dark: bool = False) -> Content:
        code = strip_terminal_control_sequences(code)
        normalized_language = cls.normalize_language(language)
        if normalized_language is None:
            return Content(code).stylize_before('$text')
        return super().highlight(code, normalized_language, ansi=ansi, dark=dark)
...
class GojeeraMarkdown(Markdown):
    ...
    def _blocks_from_tokens(self, tokens: list[Token]) -> Iterable[MarkdownBlock]:
        return self._parse_markdown(tokens)
    ...
    def update(self, markdown: str) -> AwaitComplete:
        ...
        object.__setattr__(self, '_table_of_contents', None)
        ...
        async def await_update() -> None:
            async with self.lock:
                ...
                blocks = list(self._blocks_from_tokens(tokens))
            ...
            object.__setattr__(
                self,
                '_last_parsed_line',
                len(lines) - (1 if lines and lines[-1] else 0),
            )
            self.post_message(
