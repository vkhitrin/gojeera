...
class WorkItemSearchResultsScroll(CardScrollViewMixin, ScrollView):
...
__OVERFLOW_SCROLLBAR_CSS__
    ...
    def _build_layout_for_current_geometry(self) -> tuple[int, list[SearchResultRow], int]:
        return build_scrollbar_aware_layout(
            ...
            build_rows_for_width=self._build_rows_for_width,
        )
    ...
    def _build_pending_initial_layout(self) -> tuple[int, list[SearchResultRow], int]:
    ...
    def _reset_render_state(self) -> None:
        self._reset_card_scroll_state(hide=True)
    ...
    def _finish_search_state(
        ...
        if isinstance(work_items_container, WorkItemsContainer):
            if work_items_container.results_loaded != results_loaded:
                work_items_container.results_loaded = results_loaded
            if displayed_count is not None:
                if work_items_container.displayed_count != displayed_count:
        ...
        if unified_search_bar.search_in_progress:
            unified_search_bar.search_in_progress = False
    ...
    def _blank_card_content_width(self, width: int) -> int:
        return self._card_content_width(width)
    ...
    async def action_select_work_item(self) -> None:
        if selected := self.selected_work_item:
            self._select_work_item(selected.work_item_key)
        else:
            return
    ...
    def get_work_item_by_key(self, work_item_key: str) -> JiraWorkItem | None:
        for work_item in self._work_items:
            if work_item.key == work_item_key:
                return work_item
        return None
    ...
    def _select_work_item(self, work_item_key: str) -> None:
        from gojeera.app import JiraApp
        ...
        screen.run_worker(
            screen.load_work_item(work_item_key),
            exclusive=True,
            group='work-item',
        )
