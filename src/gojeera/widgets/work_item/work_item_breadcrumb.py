...
class WorkItemBreadcrumb(Horizontal, can_focus=False):
    ...
    def _sync_parent_action_width(self) -> None:
        label_width = button_label_width(self.parent_action_widget.label)
        if label_width > 0:
            self.parent_action_widget.set_styles(f'width: {label_width}; min-width: {label_width};')
        else:
            self.parent_action_widget.set_styles('width: auto; min-width: 0;')
    ...
    def _refresh_loaded_work_item_parent_fields(
        self, screen: 'JiraApp', work_item: JiraWorkItem, parent_key: str
    ) -> None:
    ...
    async def _update_parent_work_item(self, data: dict[str, str]) -> None:
        ...
        if response.success:
            ...
            self.run_worker(
                screen.load_work_item(work_item.key),
                exclusive=True,
                group='work-item',
            )
        else:
    ...
    def set_work_item(self, work_item: JiraWorkItem | None) -> None:
        ...
        if parent_supported:
            ...
            self._sync_parent_action_width()
    ...
    async def load_parent_work_item(self) -> None:
        ...
        if callable(load_work_item):
            self.run_worker(load_work_item(parent_key), exclusive=True, group='work-item')
